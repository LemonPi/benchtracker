...
// all edit on DOM is done by populate_param_windows, which looks at params
// array
function update_params() {
    ...
    // to assure correct sequence, populate param windows must be called inside
    // the functions that update params
}
function clear_param_windows() {
    while (x_param.firstChild)
        x_param.removeChild(x_param.firstChild);
    while (y_param.firstChild)
        y_param.removeChild(y_param.firstChild);
}
function populate_param_windows() {
    ...
    if (x_sel) {
        var x;
        if (typeof x_sel === "string")
            x = x_param.querySelector([ "a[title='", x_sel, "']" ].join(""));
        else
            x = x_param.querySelector(
                [ "a[title='", x_sel.title, "']" ].join(""));
        if (x)
            toggle_x(x);
        else
            x_sel = undefined;
    ...
    if (y_sel) {
        var y;
        if (typeof y_sel === "string")
            y = y_param.querySelector([ "a[title='", y_sel, "']" ].join(""));
        else
            y = y_param.querySelector(
                [ "a[title='", y_sel.title, "']" ].join(""));
        if (y)
            toggle_y(y);
        else
            y_sel = undefined;
...
function create_filter_window(selected) {
    ...
    var param_texts = [];
    for (var p of params)
        param_texts.push(p.substring(0, p.lastIndexOf(" ")));
    var select_param =
        create_selection(param_texts, params, "--- parameters ---", false);
...
function describe_param(data) {
    ...
    if (active_querying_filter.getElementsByClassName('filter_sel').length ===
        0) {
        ...
        data_type.addEventListener('change', function() {
            ...
            mode = this.value;
            var query_param =
                active_querying_filter.getElementsByClassName("filter_param")[0]
                    .value;
            report_debug(query_param);
            $.getJSON(
                [
                  root_url, '/param?', create_task_query(), '&p=', query_param,
                  '&m=', mode
                ].join(""),
    ...
    // need to update event listener
        ...
        var data_type =
            active_querying_filter.getElementsByClassName('filter_sel')[0];
    }
...
function clean_querying_filter(data) {
    var old_filter_vals =
        active_querying_filter.getElementsByClassName("filter_val");
    while (old_filter_vals[0])
        active_querying_filter.removeChild(old_filter_vals[0]);

    var filter_sel =
        active_querying_filter.getElementsByClassName("filter_sel");
...
function create_filter_val(target, data) {
    // create either 2 selection boxes for min and max or a multi-select for in
    // filter
    var filter_val = document.createElement('div');
    filter_val.className = "filter_val";
    if (data.type === "categorical") {
        var select_in = create_selection(
            data.val, data.val,
            "ctrl+click for multiple, shift+click for range", true);
...
function toggle_task(task) {
    ...
    if (task.className === "task") {
        ...
        task.className = "task sel_task";
    } else {
        tasks.delete(task.title);
        task.className = "task";
    }
    update_params();
}
function toggle_y(selected_y) {
    if (selected_y.className === "param") {
        if (y_sel)
            y_sel.className = "param";
        y_sel = selected_y;
        selected_y.className = "param sel_param";
    } else {
        selected_y.className = "param";
        y_sel = "";
    }
}
function toggle_x(selected_x) {
    if (selected_x.className === "param") {
        if (x_sel)
            x_sel.className = "param";
        x_sel = selected_x;
        selected_x.className = "param sel_param";
    } else {
        selected_x.className = "param";
        x_sel = "";
    }
}
...
function parse_filter(filter) {
    ...
    if (filter_type === "categorical") {
        ...
        if (!min || !max) {
            report_debug("min or max missing");
            return [];
        }
...
function query_param() {
    ...
    active_querying_filter = this.parentNode;
    $.getJSON(
        [ root_url, '/param?', create_task_query(), '&p=', param ].join(""),
        describe_param, false);
...
function create_data_query() {
    ...
    if (!x_sel.title) {
        report_error("x parameter not selected");
        return '';
    }
    if (!y_sel.title) {
        report_error("y parameter not selected");
        return '';
    }
...
function report_error(error) { alert(error); }
function report_debug(debug) { console.log(debug) }
