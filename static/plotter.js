...
errMsg = {
    ENUMPLOTS :
        'More than' + maxPlotsAllowed + ' will be generated.\n' +
...
// Some const
// if x axis is an element of timeParam, then the default behavior is to gmean
// everything
...
var overlay_list = [];
var gmean_list = [];
// raw data means it is not processed by gmean, and its param name has not been
// manipulated.
// NOTE: we don't store the data of default plot (cuz they are not manipulated)
var raw_data = null;
// store data after gmean
// each task has a folder, each plot has a subfolder, each line has a file
...
var formGmean;
var formOverlay;
// cached overlay selector choices so that if the choices are the same, we
// default to picking the same ones
...
function reduceToGmean(groupedX) {
    ...
    var validCount = 0;
    groupedX = _.reduce(groupedX, function(memo, num) {
        if (num > 0) {
            memo *= num;
            validCount += 1;
        }
    ...
    return (validCount > 0) ? Math.pow(groupedX, 1.0 / validCount) : invalidY;
}
/*
 * reduce a list o a single arithmetic mean value
 */
function reduceToAmean(groupedX) {
    groupedX = _.map(groupedX, function(list) { return list[1]; });
    var numElements = groupedX.length;
    return _.reduce(groupedX, function(memo, num) { return memo + num; }) /
           numElements;
}

var reduceToMean = null;
...
$(document)
    .ready(function() {
        ...
        formGmean = document.getElementById("gmean_select")
                        .getElementsByTagName("fieldset")[0];
        formOverlay = document.getElementById("overlay_select")
                          .getElementsByTagName("fieldset")[0];
        ...
        var geometricMeanSelect = document.getElementById("geometric_mean");
        var arithmeticMeanSelect = document.getElementById("arithmetic_mean");

        geometricMeanSelect.addEventListener('click', function() {
            reduceToMean = reduceToGmean;
            geometricMeanSelect.setAttribute('class', 'active_mean_method');
            arithmeticMeanSelect.setAttribute('class', 'inactive_mean_method');
        });
        arithmeticMeanSelect.addEventListener('click', function() {
            reduceToMean = reduceToAmean;
            arithmeticMeanSelect.setAttribute('class', 'active_mean_method');
            geometricMeanSelect.setAttribute('class', 'inactive_mean_method');
        });
        geometricMeanSelect.click();
    });
...
function plotter_setup(data) {
    ...
    raw_data = data;
    raw_data.tasks =
        _.map(raw_data.tasks, function(d) { return d.split('|')[0]; });
    gmean_data = null;
    overlay_list = [];
    gmean_list = [];
    range = [];
...
function defaultToGmeanTimePlot() {
    ...
    for (var i = 0; i < raw_data.data.length; i++) {
        groupedX = _.groupBy(raw_data.data[i],
                             function(dotSeries) { return dotSeries[0] });
        seriesXY = [];
        for (var k in groupedX) {
            groupedX[k] = reduceToMean(groupedX[k]);
            // now {x1: y1, x2: y2, ...}
 ...
 * methodology: gmean over circuits, overlay on the filter which has the most
 * distinct values
 */
function defaultToGmeanSubPlot() {
    ...
    if (gmeanIndex == -1) {
        ...
        for (var k in raw_data.data) {
            var data = [];
            gmeanPrepData = data_transform(
                raw_data.data[k], [ (gmeanIndex - 2) + '' ], 'non-overlay');
            for (var p in gmeanPrepData) {
                // now gmeanPrepData[p] is a subplot overlaying on circuit
                // next: gruop on x and gmean on y
                var groupedX = data_transform(gmeanPrepData[p],
                                              [ (0 - 2) + '' ], 'xy group');
                var seriesXY = [];
                for (var l in groupedX) {
                    groupedX[l] = reduceToMean(groupedX[l]);
            ...
            for (var i in data) {
                ...
                for (var j in data[i]) {
                    ...
                    data[i][j] =
                        [ data[i][j][0], data[i][j][1] ].concat(paramSplit);
                    newRawData.push(data[i][j]);
                }
            }

            newOverlayList =
                [ findMostExpensiveAxis(newRawData, newParams) + '' ];
            var newData =
                data_transform(newRawData, newOverlayList, 'non-overlay');
            ...
            if (count <= maxPlotsAllowed) {
                for (var j in newData) {
                    simple_plot(newParams, newData[j], newOverlayList,
                                xNameMap[k], k, 'normalTitle');
                }
            } else {
                count = 0;
...
function generate_overlay_selector() {
    ...
    for (var i = 2; i < choice.length; i++) {
        ...
        input.addEventListener('change',
                               function() { updateLegendCheckBox(this.id); });
    ...
    function updateLegendCheckBox(gid) {
        ...
        if (d3.select('#' + gid).property('checked')) {
            ...
            d3.select('#olabel-' + oid)
                .attr('style', 'color: rgb(188,188,188)');
        } else {
...
function plot_generator() {
    ...
    // filter is an array of array: filter[i][*] stores all possible values of
    // axis i
    ...
    for (var i in series) {
        ...
        for (var k in gmeanPrep) {
            var groupedX =
                data_transform(gmeanPrep[k], [ (0 - 2) + '' ], 'xy group');
            seriesXY = [];
            for (var l in groupedX) {
                groupedX[l] = reduceToMean(groupedX[l]);
    ...
    for (var i = 0; i < series.length; i++) {
        var grouped_series =
            data_transform(series[i], overlay_list, 'non-overlay');
        range.push({x : [], y : []});
        range[i]['x'] = [ Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY ];
        range[i]['y'] = [ Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY ];
        manipulatedData[raw_data.tasks[i]] = {};
        ...
        var temp = _.map(gmean_list, function(d) {
                        return raw_data.params[Number(d) + 2];
                    }).join();
        if (temp == '') {
...
function sizingTaskContainer(numPlots, id) {
    ...
    var wholeWidth =
        document.getElementById('display_canvas').offsetWidth / numPlotsPerRow;
...
function findAxisScale(series, xNM, t) {
    for (var i = 0; i < series.length; i++) {
        if (xNM.values.length == 0) {
            range[t]['x'][0] = (range[t]['x'][0] > Number(series[i][0]))
                                   ? Number(series[i][0])
                                   : range[t]['x'][0];
            range[t]['x'][1] = (range[t]['x'][1] < Number(series[i][0]))
                                   ? Number(series[i][0])
                                   : range[t]['x'][1];
        }
        if (Number(series[i][1]) != invalidY) {
            range[t]['y'][0] = (range[t]['y'][0] > Number(series[i][1]))
                                   ? Number(series[i][1])
                                   : range[t]['y'][0];
            range[t]['y'][1] = (range[t]['y'][1] < Number(series[i][1]))
                                   ? Number(series[i][1])
                                   : range[t]['y'][1];
        }
    }
...
function data_transform(series, overlay_list, mode) {
    // group by
    if (mode == 'non-overlay') {
        return _.groupBy(series, function(dot_info) {
            axis_group = "";
...
function prepareData(params, series, overlay_list, xNM, t, titleMani) {
        ...
        for (var j in manipulatedData[i]) {
            for (var k in manipulatedData[i][j]) {
                for (var m in manipulatedData[i][j][k]) {
                    manipulatedData[i][j][k][m] = [
                        manipulatedData[i][j][k][m][0],
    ...
    for (var k in lineData) {
        ...
        if (xNM.values.length == 0) {
            ...
            // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
            lineVal = _.map(lineVal, function(d) {
                return {
                    x : epochTimeConverter(d.x, params[0]),
                    y : epochTimeConverter(d.y, params[1])
                };
            });
        } else {
            ...
            lineVal = _.map(lineVal, function(d) {
                return {
                    x : xNM.values.indexOf(d.x),
                    y : epochTimeConverter(d.y, params[1])
                };
            });
            lineVal = _.sortBy(lineVal, 'x');
            lineVal = _.map(lineVal, function(d) {
                return {x : xNM.values[d.x], y : d.y};
            });
        }
        if (isNaN(Number(k))) {
            lineInfo.push({values : lineVal, key : k});
        } else {
            // TODO: this Number() won't work if k is a combination of numerical
            // and non-numerical
...
function setupAxis(params, xNM, t, width, height) {
    ...
    if (xNM.values.length == 0) {
        var xAxisRange = [
            range[t]['x'][0] - 0.1 * (range[t]['x'][1] - range[t]['x'][0]),
            range[t]['x'][1] + 0.1 * (range[t]['x'][1] - range[t]['x'][0])
        ];
        ...
        if (unixEpoch.indexOf(params[0]) == -1) {
            x = d3.scale.linear().domain(xAxisRange).range([ 0, width ]);
        } else {
            x = d3.time.scale().domain(xAxisRange).range([ 0, width ]);
        }
    } else {
        x = d3.scale.ordinal()
                .domain(xNM.values)
    ...
    var y = null;
    var yAxisRange = [
        range[t]['y'][0] - 0.2 * (range[t]['y'][1] - range[t]['y'][0]),
        range[t]['y'][1] + 0.2 * (range[t]['y'][1] - range[t]['y'][0])
    ];
    ...
    if (unixEpoch.indexOf(params[1]) == -1) {
        y = d3.scale.linear().domain(yAxisRange).range([ height, 0 ]);
    } else {
        y = d3.time.scale().domain(yAxisRange).range([ height, 0 ]);
    }
    return [ x, y ];
}
/*
...
function simple_plot(params, series, overlay_list, xNM, t, titleMode) {
    ...
    // width & height is the size of the actual plotting area (i.e.: rect in
    // canvas svg)
    var width = canvWidth -
                wholeWidth * (ratio.marginLefToWhole + ratio.marginRigToWhole);
    var height =
        canvHeight -
        wholeHeight * (ratio.marginTopToWhole + ratio.marginBotToWhole);
    ...
    */
    ...
    var xAxis = d3.svg.axis().scale(x).orient("bottom").ticks(10, 's').tickSize(
        -height);
    var yAxis =
        d3.svg.axis().scale(y).orient("left").ticks(10, 's').tickSize(-width);
    // ...................................
    // behavior
    var zoom =
    ...
    // assembly
    var chart =
        d3.select('#task_' + t).append('div').attr('class', 'chart_container');
    ...
    // NB: the translation on g is not strict. By inspecting the actual
    // generated webpage,
    // it seems that the translation of (gTransX, gTransY) is eventually falling
    // on rect.
    ...
    var xParam = _.reduce(params[0].split('_'), function(memo, num) {
        return memo == '' ? num : memo + ' ' + num;
    }, '');
    var yParam = _.reduce(params[1].split('_'), function(memo, num) {
        return memo == '' ? num : memo + ' ' + num;
    }, '');
    // the hardcoded 30 in "attr('y', height+30)" should be fine. cuz vertical
    // px is irrelevant to window size
    svg.append('text')
        .attr('class', 'x label')
    ...
    // add line and dots
    var lineGen =
        d3.svg.line().x(function(d) { return x(d['x']); }).y(function(d) {
            return y(d['y']);
        });
    ...
    svg.append('rect').attr('class', 'overlay').attr('width',
    ...
    */
    ...
    var svgLegend = chart.append('div')
                        .attr('class', 'legend_container')
                        .style('width', divLegWidth)
                        .style('height', divLegHeight)
                        .append('svg')
                        .attr('class', 'legend_svg');
    // NOTE: has to set font here, or getBBox will return inprecise value due to
    // the unloaded font
    ...
    if (lineInfo.length > 1 || lineInfo[0]['key'] != '') {
        var legend = svgLegend.selectAll('.legend')
                         ...
                         .attr('transform', function(d, i) {
                             ...
                             var vert = (i + 1) * height +
                                        wholeHeight * ratio.marginTopToWhole +
                                        legendPeriGap;
                             return 'translate(' + horz + ', ' + vert + ')';
                         });
        var legendTitle = _.map(overlay_list, function(d) {
            return '<' + params[Number(d) + 2] + '>';
        });
        for (var i in legendTitle) {
            legendTitle[i] =
                _.reduce(legendTitle[i].split('_'), function(memo, num) {
                    return memo == '' ? num : memo + ' ' + num;
                }, '');
        }
        ...
        var textElement =
            svgLegend.append('text')
                .attr('x', legendPeriGap)
                .attr('y', legendPeriGap + wholeHeight * ratio.marginTopToWhole)
                .attr('text-anchor', 'left')
                .style('font-size', '12px')
                .style('font-weight', 'bold')
                .text(String(legendTitle));
        // dynamically adjust the svg legend size
        svgLegWidth = (svgLegWidth > textElement.node().getBBox().width)
                          ? svgLegWidth
        ...
        legend.append('rect')
            ...
            .style('stroke', color);
        legend.append('text')
            .attr('x', legendSize + legendMargin)
            .attr('y', legendSize - legendMargin)
            .style('font-size', '12px')
            .text(function(d) { return d; });
        legend.each(function() {
            svgLegWidth = (svgLegWidth > this.getBBox().width)
                              ? svgLegWidth
                              : this.getBBox().width;
        ...
        svgLegHeight +=
            legendPeriGap + wholeHeight * ratio.marginTopToWhole + legendSize;
    ...
    for (var i = 2; i < series[0].length; i++) {
        if ($.inArray(i - 2 + '', overlay_list) == -1 &&
            ...
            if (titleMode == 'normalTitle') {
                paramRejoin =
                    _.reduce(params[i].split('_'), function(memo, num) {
                        return memo == '' ? num : memo + ' ' + num;
                    }, '');
    ...
    // have to append this rect after the plots, in order for the peripheral
    // stroke
...
function savePlot() {
    var count = 0;
    // var all is for compatibility of firefox. FF won't load img correctly
    // unless you call onload function
    ...
    d3.selectAll('.chart_container').each(function() {
        var html1 = d3.select(this)
                        .select('svg')
                        .attr('version', 1.1)
                        .attr('xmlns', 'http://www.w3.org/2000/svg')
                        .node()
                        .parentNode.innerHTML;
        var html2 = d3.select(this)
        ...
        var plotWidth =
            Number(d3.select(this).select('.canvas_container').attr('width'));
        var plotHeight =
            Number(d3.select(this).select('.canvas_container').attr('height'));
        var legendWidth =
            Number(d3.select(this).select('.legend_svg').attr('width'));
        var legendHeight =
            Number(d3.select(this).select('.legend_svg').attr('height'));
        ...
        image.onload = function() {
            ...
            */
            zip.file('png/plotPng-' + count + '.png',
                     canvasdata.substr(canvasdata.indexOf(',') + 1),
                     {base64 : true});
            zip.file('svg/plotSvg-' + count + '.svg', btoa(html),
                     {base64 : true});
            if (count == all) {
